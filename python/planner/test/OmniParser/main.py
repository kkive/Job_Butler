...
def build_paths(base_dir: Path, image_path: str | None = None) -> tuple[Path, Path]:
    resolved_image_path = Path(image_path).resolve() if image_path else (base_dir / "./image.png")
    yolo_model_path = base_dir / "weights" / "icon_detect" / "model.pt"
    return resolved_image_path, yolo_model_path


def main(image_path: str | None = None, include_annotated_image_base64: bool = False) -> dict:
    base_dir = Path(__file__).resolve().parent
    image_path, yolo_model_path = build_paths(base_dir, image_path)

    if not image_path.exists():
        raise FileNotFoundError(f"未找到待解析图片文件: {image_path}")
